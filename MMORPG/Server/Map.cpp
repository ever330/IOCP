...
void Map::Initialize()
{
	...
	{
		// 다음 몬스터 ID 부여. Map 정보를 DB에서 읽어올 때 몬스터 정보도 같이 읽어야 함
...
void Map::Update(float deltaTime, int tickCount)
{
	...
	for (int userID : m_users)
	{
		...
		user->GetCharacter().Update(deltaTime); // 캐릭터 상태 업데이트 (쿨타임 등)
	}
	...
	// 10틱마다 유저 캐릭터 상태 브로드캐스트
	if (tickCount % 10 == 0)
	{
		PlayerStateUpdate();  // 유저 캐릭터 상태 전체 브로드캐스트
	}
...
void Map::PlayerAttack(std::shared_ptr<User> user, C2SPlayerAttackPacket pac)
{
	...
	Direction dir = (Direction)pac.AttackDirection; // 패킷에서 방향을 추출해서
	...
	for (auto& monster : m_monsters)
	{
		...
		if (hitBox.Contains(monPos))
		{
			...
			if (expOpt.has_value())
			{
				...
				user->GetCharacter().AddExp(expGained);  // 경험치 추가
